...
export function optionsSetTransform(value: TransformValueType) {
  const ytmeRoot = document.querySelector('[ytme-root]') as HTMLElement
  if (!ytmeRoot) return
  ytmeRoot.style.transform = `translate(${value.translate.x}px, ${value.translate.y}px) rotate(${value.rotate}deg) scale(${value.scale})`
}
export function setTransform(value: TransformValueType) {
  ...
  if (videoParentElement) {
    videoParentElement.style.transform = `translate(${value.translate.x}px, ${value.translate.y}px) rotate(${value.rotate}deg) scale(${value.scale})`
  }
  ...
  if (restoreButton) {
    if (value.translate.x === 0 && value.translate.y === 0 && value.rotate === 0 && value.scale === 1) {
...
export function rotateVideoParent(degree: 0 | 90 | 270) {
  updateTransform({
